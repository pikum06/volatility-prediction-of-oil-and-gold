...
OIL_ASSETS = {
    "BZ=F": {"name": "Brent Crude Oil Futures", "p": 1, "q": 2},
    "QM=F": {"name": "E-mini Crude Oil Futures", "p": 1, "q": 0},
    "CRUD.MI": {"name": "WisdomTree WTI Crude Oil ETF", "p": 1, "q": 1},
}

#Date Range for Analysis

START_DATE_OIL = "2014-01-01"
END_DATE_OIL = "2022-01-01"


# Function to Analyze Oil Volatility

def oil_volatility():
    for ticker, config in OIL_ASSETS.items():
        print(f"\nProcessing Oil Asset: {config['name']} ({ticker})...")

        # Data Acquisition & Log Returns

        df = yf.download(ticker, start=START_DATE_OIL, end=END_DATE_OIL)
